...
class _ShowDetailScreenState extends State<ShowDetailScreen> {
  ...
  Future<void> fetchTrailer() async {
    try {
      final url = Uri.parse(
        '${ApiConfig.baseUrl}/tv/${widget.show['id']}/videos?api_key=${ApiConfig.apiKey}',
      );
  ...
  Future<void> _launchTrailer() async {
    if (trailerKey == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("Trailer not found.")));
      return;
    }
  ...
  Future<void> fetchFullDetails() async {
    try {
      final url = Uri.parse(
        '${ApiConfig.baseUrl}/tv/${widget.show['id']}?api_key=${ApiConfig.apiKey}',
      );
      final res = await http.get(url);
      if (res.statusCode == 200) {
          ...
          setState(() {
            ...
            if (fullDetails!['seasons'].isNotEmpty) {
              selectedSeasonNumber =
                  fullDetails!['seasons'][0]['season_number'];
  ...
  Future<void> fetchEpisodes(int sNum) async {
    ...
    try {
      final url = Uri.parse(
        '${ApiConfig.baseUrl}/tv/${widget.show['id']}/season/$sNum?api_key=${ApiConfig.apiKey}',
      );
      final res = await http.get(url);
      if (res.statusCode == 200) {
        if (mounted)
      ...
      if (mounted) setState(() => isEpisodesLoading = false);
    }
  }
  ...
  Future<void> fetchSeriesCast() async {
    try {
      final url = Uri.parse(
        '${ApiConfig.baseUrl}/tv/${widget.show['id']}/aggregate_credits?api_key=${ApiConfig.apiKey}',
      );
      final res = await http.get(url);
      if (res.statusCode == 200) {
        if (mounted)
          setState(
            () => seriesCast = json.decode(res.body)['cast'].take(12).toList(),
          );
      }
    } catch (e) {
    ...
    try {
      final p = await PaletteGenerator.fromImageProvider(
        CachedNetworkImageProvider(
          "https://image.tmdb.org/t/p/w342${widget.show['poster_path']}",
        ),
      );
      if (mounted)
        setState(
          () => activeColor =
              p.dominantColor?.color.withValues(alpha: 0.2) ??
            ...
            return Column(
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    if (user != null) {
                      final data = {
                        'user_id': user.id,
                        'show_id': showId,
                        'content': reviewController.text.trim().isEmpty
                  ...
                  child: Text(
                    myReviewId != null ? "UPDATE REVIEW" : "SAVE REVIEW",
                    style: const TextStyle(
                      color: AppColors.textPrimary,
                      fontWeight: FontWeight.bold,
                    ),
                    ...
                    background: Stack(
                      ...
                      children: [
                        if (widget.show['backdrop_path'] != null)
                          CachedNetworkImage(
                            imageUrl:
                                "https://image.tmdb.org/t/p/w780${widget.show['backdrop_path']}",
                            fit: BoxFit.cover,
                          ),
                        Container(
                          decoration: BoxDecoration(
  ...
  Widget _buildHeader() {
    String releaseYear =
        (fullDetails != null &&
            fullDetails!['first_air_date'] != null &&
            fullDetails!['first_air_date'] != "")
        ? " (${fullDetails!['first_air_date'].split('-')[0]})"
        : "";
    return Row(
      children: [
        ClipRRect(
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Text(
                    widget.show['vote_average']?.toStringAsFixed(1) ?? 'N/A',
                    style: const TextStyle(
                      color: AppColors.textPrimary,
                      fontWeight: FontWeight.bold,
                    ),
              ...
              Row(
                children: [
                  _actionBtn(
                    ...
                    isLiked ? AppColors.accent : AppColors.textSecondary,
                    _toggleLike,
                  ),
                  const SizedBox(width: 12),
                  _actionBtn(
                    isWatched ? Icons.check_circle : Icons.visibility_outlined,
                    isWatched ? AppColors.accent : AppColors.textSecondary,
                    _toggleWatched,
                  ),
                  const SizedBox(width: 12),
                  _actionBtn(
                    Icons.star_rate_rounded,
                    myRating > 0 ? AppColors.accent : AppColors.accentSecondary,
                    () => _openReviewModal(
                      context,
                      widget.show['id'],
                      widget.show['name'],
  ...
  Widget _buildRatingsSection() {
    int maxCount = ratingDistribution.reduce(max);
        ...
        Row(
          ...
          children: [
            ...
            Column(
              ...
              children: [
                Text(
                  showEpoint > 0 ? (showEpoint / 2).toStringAsFixed(1) : "0.0",
                  style: const TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 28,
  ...
  Widget _buildCommunitySection() => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text(
        "COMMUNITY",
          ...
          Expanded(
            child: _communityBtn(
              ...
              () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (c) => ShowReviewsScreen(
                    showId: widget.show['id'],
                    showName: widget.show['name'],
                    posterPath: widget.show['poster_path'],
                  ),
                ),
              ),
            ),
  ...
  Widget _communityBtn(String t, IconData i, VoidCallback o) => GestureDetector(
    onTap: o,
    child: Container(
  ...
  Widget _actionBtn(IconData i, Color c, VoidCallback t) => GestureDetector(
    onTap: t,
    child: Container(
  ...
  Widget _sectionHeader(String t, VoidCallback o) => Row(
  ...
  Widget _buildCastList() => SizedBox(
    height: 120,
    child: ListView.builder(
      scrollDirection: Axis.horizontal,
      itemCount: seriesCast.length,
      itemBuilder: (context, i) {
  ...
  Widget _buildSeasonPicker() {
    final seasons = fullDetails?['seasons'] ?? [];
    return SizedBox(
      height: 35,
      child: ListView.builder(
        scrollDirection: Axis.horizontal,
        ...
        itemBuilder: (context, i) {
          final s = seasons[i];
          final isSel = selectedSeasonNumber == s['season_number'];
          return GestureDetector(
            onTap: () {
              setState(() => selectedSeasonNumber = s['season_number']);
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isSel ? AppColors.accent : AppColors.elevated,
  ...
  Future<void> _toggleLike() async {
    ...
    try {
      if (isLiked) {
        ...
        await _supabase.from('liked_shows').insert({
          'user_id': user.id,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor: isAdding ? AppColors.accent : AppColors.surface,
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              isAdding ? "Marked as Watched! 📺" : "Removed from Watched",
              style: const TextStyle(
                color: AppColors.textPrimary,
                fontWeight: FontWeight.bold,
              ),
